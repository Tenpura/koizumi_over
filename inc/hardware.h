...
#if (MOUSE_NAME == KOIZUMI_FISH)
	#define UI_MODE_N	(9U)				/*	User Interface LED で表すモードの数	*/
...
#if (MOUSE_NAME == KOIZUMI_FISH)
	const uint16_t MAX_PERIOD = (840 - 1); //FIXME 周波数がホントは168MHz＝16800-1が正しいはず　//PWMの周波数決めのための最大カウント		84MHzで100kHzになる
...
#elif (MOUSE_NAME == KOIZUMI_OVER)
	const uint16_t MAX_PERIOD = (1680 - 1);	//PWMの周波数決めのための最大カウント	100kHzになる
...
static const unsigned int ENC_GPIO_N = ENC_N * 2;		//1つのエンコーダにつき2チャンネル使用
	...
	static const uint16_t ENC_RES = (4096U);		//エンコーダの分解能　12bit		//TODO 最適化に伴い10bitから自動計算にしたい。
	...
	static const uint16_t ENC_RES = (1024U);		//エンコーダの分解能　10bit	//TODO 最適化に伴い10bitから自動計算にしたい。
...
private:
	...
	static float correct[2][ ENC_RES + 1 ];	//補正用のテーブル
...
public:
	...
	static int16_t raw_cnt_watch[2];	//エンコーダ―の生値観測用
